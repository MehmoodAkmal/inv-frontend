...
function RoleCard({ role, permissions, catalog, custom, onToggle }) {
  ...
  const enabledCount = Object.entries(catalog).reduce(
    (total, [module, actions]) =>
      total + actions.filter((action) => permissions?.[module]?.[action]).length,
    0
  );
  ...
  return (
    <section className="card overflow-hidden">
      <div className="px-5 py-4 bg-gradient-to-r from-primary-50 to-white border-b border-primary-100 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900 capitalize">{role}</h2>
          <p className="mt-0.5 text-xs text-gray-500">
            {custom ? 'Custom organization defaults' : 'Built-in system defaults'}
          </p>
        </div>
        <span className="shrink-0 rounded-full bg-white border border-primary-100 px-2.5 py-1 text-xs font-semibold text-primary-700">
                ...
                <span className="flex items-center gap-2">
                  ...
                  >
                    <path
                      ...
                      d={isOpen ? 'M5 9l7 7 7-7' : 'M9 5l7 7-7 7'}
                    />
                  </svg>
              ...
              {isOpen && (
                <div className="px-5 pb-4 grid grid-cols-2 gap-x-4 gap-y-2.5">
                  {actions.map((action) => (
                    <label
                      key={action}
                      className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm text-gray-700 cursor-pointer hover:bg-primary-50"
                    >
                      <input
                        type="checkbox"
                        checked={Boolean(permissions?.[module]?.[action])}
                        onChange={() => onToggle(role, module, action)}
...
export default function Permissions() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRolePermissions({
        manager: results[0].data.data.permissions,
        cashier: results[1].data.data.permissions,
      });
      setCatalog(results[0].data.data.catalog);
      setCustomRoles({
        manager: results[0].data.data.hasCustomDefault,
        cashier: results[1].data.data.hasCustomDefault,
      });
    } catch (err) {
  ...
  const toggle = (role, module, action) =>
    setRolePermissions((current) => ({
      ...
      [role]: {
        ...current[role],
        [module]: { ...current[role]?.[module], [action]: !current[role]?.[module]?.[action] },
      },
    }));
  const save = async () => {
    setSaving(true);
    try {
      await Promise.all(ROLES.map((role) => updateRolePermissions(role, rolePermissions[role])));
      setCustomRoles({ manager: true, cashier: true });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Set default access for new managers and cashiers. Individual users can have custom
            overrides.
          </p>
        </div>
        <button className="btn-primary shrink-0" onClick={save} disabled={loading || saving}>
          {saving && <Spinner size="sm" className="mr-2" />}Save defaults
        </button>
      </div>
      {loading ? (
