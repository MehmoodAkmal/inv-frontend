...
function CustomerDetailDrawer({
  ...
  const handlePayment = async (e) => {
    ...
    try {
      const branchId =
        userRole === 'admin' ? (customer.branchId?._id ?? customer.branchId) : allowedBranchId;
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white h-full flex flex-col shadow-card-lg overflow-hidden">
        ...
        {/* ── Balance banner ───────────────────────────────────────── */}
        <div
          className={`px-6 py-4 flex items-center justify-between ${balance > 0 ? 'bg-amber-50 border-b border-amber-100' : 'bg-emerald-50 border-b border-emerald-100'}`}
        >
          <div>
          ...
          <div className="flex flex-col gap-2 items-end">
            ...
            <span
              className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold ${customer.isActive ? 'badge-active' : 'badge-inactive'}`}
            >
        ...
        {showPay && (
          <form
            ...
            className="px-6 py-4 bg-white border-b border-brand-100 space-y-3"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-6 py-3 flex items-center justify-between sticky top-0 bg-white border-b border-brand-100 z-10">
            <h3 className="text-xs font-bold text-brand-700 uppercase tracking-wider">
              Ledger History
              {ledgerPag && (
                <span className="ml-2 font-normal text-brand-400">({ledgerPag.total} entries)</span>
              )}
            </h3>
          </div>
            ...
            <div className="px-6 py-4 space-y-2">
              ...
              {ledgerPag && ledgerPag.totalPages > 1 && (
                <div className="flex items-center justify-between pt-3 text-xs text-brand-500">
                  <span>
                    Page {page} of {ledgerPag.totalPages}
                  </span>
                  <div className="flex gap-2">
                    ...
                    <button
                      className="btn-secondary py-1 px-3 text-xs"
                      disabled={page >= ledgerPag.totalPages}
                      onClick={() => setPage((p) => p + 1)}
                    >
                      Next
                    </button>
...
export default function Customers() {
  ...
  const totalOutstanding = customers.reduce(
    (s, c) => s + (c.currentBalance > 0 ? c.currentBalance : 0),
    0
  );
      ...
      {!loading && customers.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            {
              label: 'Outstanding',
              value: `${Number(totalOutstanding).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`,
              color: 'bg-rose-400',
            },
          ].map((s) => (
            <div key={s.label} className="card p-4 relative overflow-hidden">
              ...
              <p className="text-[10px] font-bold text-brand-500 uppercase tracking-wider pl-3">
                {s.label}
              </p>
              <p className="text-2xl font-extrabold text-brand-900 tracking-tight pl-3 mt-0.5">
                {s.value}
              </p>
            </div>
          ))}
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-desc">
              {search ? `No results for "${search}"` : 'Add your first credit customer'}
            </p>
          </div>
        ) : (
