...
function NewSaleForm({ branches, items, customers, userRole, allowedBranchId, onSubmit, onCancel, saving }) {
  ...
  const subtotal    = round2(lines.reduce((s, l) => s + round2(Number(l.quantity || 0) * Number(l.sellingPrice || 0)), 0));
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          {["cash", "credit"].map((t) => (
            <label key={t} className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio" name="paymentType" value={t}
                checked={form.paymentType === t}
                onChange={() => { set("paymentType", t); set("customerId", ""); }}
      ...
      <div>
        ...
        <CustomSelect
          ...
          className="input-field"
        >
          <option value="">{form.paymentType === "cash" ? "No customer (optional)" : "Select customer"}</option>
          {customers.map((c) => (
      ...
      <div>
        <label className="label">
          ...
          {form.paymentType === "cash" && <span className="ml-1 text-xs text-gray-400">(auto-filled for cash)</span>}
        </label>
        <input
          type="number" min="0" step="0.01" required
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          {sale.customerId?.phone && <p className="text-gray-400 text-xs">{sale.customerId.phone}</p>}
        </div>
        <div>
      ...
      <div className="bg-gray-50 rounded-lg p-3 space-y-1 text-sm">
        ...
        <div className="flex justify-between text-gray-600">
          <span>Amount paid</span><span>{fmt(sale.amountPaid)}</span>
        </div>
        {sale.balanceDue > 0 && (
          <div className="flex justify-between font-semibold text-amber-700">
            <span>Balance due</span><span>{fmt(sale.balanceDue)}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-4">
        ...
        {(filterBranch || filterType || filterStart || filterEnd) && (
          <button className="btn-secondary text-sm py-2" onClick={() => {
            setFilterBranch(""); setFilterType(""); setFilterStart(""); setFilterEnd(""); setPage(1);
          }}>
            Clear
          </button>
        )}
          ...
          <>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {["Date", "Customer", "Type", "Total", "Paid", "Balance Due", "By", ""].map((h) => (
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {sales.map((sale) => (
                    <tr key={sale._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3"><PaymentBadge type={sale.paymentType} /></td>
                      <td className="px-4 py-3 text-sm font-semibold text-gray-900">{fmt(sale.totalAmount)}</td>
