      ...
      <div className="w-full max-w-lg bg-white h-full flex flex-col shadow-card-lg overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {/* Status banner */}
            <div className={`px-6 py-3 flex items-center justify-between border-b ${detail.isActive ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100"}`}>
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="card p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-brand-400 mt-1">Max branches: <span className="font-semibold text-brand-700">{detail.maxBranches}</span></p>
                </div>
              ...
              <div>
                <p className="text-xs font-bold text-brand-700 uppercase tracking-wider mb-2">
                  Branches <span className="text-brand-400 font-normal normal-case">({detail.branches.length})</span>
                </p>
                  ...
                  <div className="space-y-1.5">
                    {detail.branches.map(b => (
                      <div key={b._id} className="flex items-center justify-between px-3 py-2 bg-brand-50 rounded-lg">
                        <div className="flex items-center gap-2">
                          <svg className="w-3.5 h-3.5 text-brand-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div>
                <p className="text-xs font-bold text-brand-700 uppercase tracking-wider mb-2">
                  Users <span className="text-brand-400 font-normal normal-case">({detail.users.length})</span>
                </p>
                {detail.users.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {detail.users.map(u => (
                      <div key={u._id} className="flex items-center gap-3 px-3 py-2.5 bg-brand-50 rounded-lg">
...
export default function SuperAdminOrganizations() {
  ...
  const handlePlanUpdated = (orgId, data) =>
    setOrgs(prev => prev.map(o =>
      o._id === orgId ? { ...o, subscriptionPlan: data.subscriptionPlan, maxBranches: data.maxBranches } : o
    ));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-brand-900 leading-none">Organizations</h1>
          <p className="text-sm text-brand-500 mt-1.5">
            {pagination ? `${pagination.total} organization${pagination.total !== 1 ? "s" : ""} registered` : "Loading…"}
          </p>
        </div>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-brand-100">
            <thead>
              <tr>
                {["Organization","Plan","Subscription Status","Active","Branches","Users","Joined",""].map(h => (
              ...
              {loading ? (
                ...
                >
                  <td className="table-td">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white shrink-0 ${org.isActive ? "bg-primary-600" : "bg-brand-400"}`}>
                  ...
                  <td className="table-td">
                    ...
                    <div className="mt-1 h-1 w-14 bg-brand-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.min((org.branchCount / org.maxBranches) * 100, 100)}%` }}
                      />
                    </div>
                  </td>

                  <td className="table-td text-sm font-medium text-brand-700">
                    {org.userCount}
                  </td>
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-brand-100 text-xs text-brand-500">
            ...
            <div className="flex gap-2">
              ...
              <button className="btn-secondary py-1 px-3 text-xs" disabled={page >= pagination.totalPages}
