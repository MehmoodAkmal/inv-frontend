...
function RecordPaymentForm({ employees, userRole, allowedBranchId, onSubmit, onCancel, saving }) {
  ...
  const availableEmployees = userRole === "manager"
    ? employees.filter((e) => e.branchId === allowedBranchId || e.branchId?._id === allowedBranchId)
    : employees;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input name="month" type="month" required value={form.month} onChange={change} className="input-field" />
        <p className="text-xs text-gray-400 mt-1">If a payment already exists for this month, the amount will be added to it.</p>
      </div>
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track monthly salary payments for each employee</p>
        </div>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-4">
        ...
        {(filterBranch || filterMonth || filterEmployee) && (
          <button className="btn-secondary text-sm py-2" onClick={() => {
            setFilterBranch(""); setFilterMonth(""); setFilterEmployee(""); setPage(1);
          }}>Clear</button>
        )}
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {["Employee","Designation","Month","Amount Paid","Monthly Salary","Paid On","Status","Note","By"].map((h) => (
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {payments.map((p) => (
                    <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{p.employeeId?.name ?? "—"}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{p.employeeId?.designation || "—"}</td>
                      <td className="px-4 py-3 text-sm text-gray-700 font-mono">{p.month}</td>
                      <td className="px-4 py-3 text-sm font-semibold text-gray-900">{fmt(p.amount)}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{fmt(p.employeeId?.monthlySalary)}</td>
                      <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{fmtd(p.paidOn)}</td>
                      <td className="px-4 py-3"><StatusBadge status={p.status} /></td>
                      <td className="px-4 py-3 text-sm text-gray-400 max-w-[120px] truncate">{p.note || "—"}</td>
