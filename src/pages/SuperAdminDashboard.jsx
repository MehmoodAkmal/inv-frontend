      ...
      <div className="w-full max-w-lg bg-white h-full flex flex-col shadow-card-lg overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Status banner */}
            <div
              className={`px-6 py-3 flex items-center justify-between text-sm border-b ${detail.isActive ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}
            >
                ...
                {detail.users.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {detail.users.map((u) => (
                      <div
                        key={u._id}
                        className="flex items-center gap-3 px-3 py-2 bg-brand-50 rounded-lg"
                      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5">
          ...
          {activeLoad ? (
            ...
            <div className="h-44 flex items-center justify-center text-xs text-brand-400">
              No sales data
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={176}>
      ...
      <div className="card overflow-hidden">
                  ...
                  >
                    <td className="table-td">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white shrink-0 ${org.isActive ? 'bg-primary-600' : 'bg-brand-400'}`}
                        >
        ...
        {orgPag && orgPag.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-brand-100 text-xs text-brand-500">
            <span>
              Page {page} of {orgPag.totalPages} · {orgPag.total} organizations
            </span>
            <div className="flex gap-2">
              ...
              <button
                className="btn-secondary py-1 px-3 text-xs"
                disabled={page >= orgPag.totalPages}
                onClick={() => setPage((p) => p + 1)}
              >
                Next
              </button>
