...
function RoleBadge({ role }) {
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${ROLE_STYLE[role] ?? 'bg-brand-100 text-brand-600'}`}
    >
      {role}
    </span>
  );
...
export default function SuperAdminUsers() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-brand-500 mt-1.5">
            {pagination
              ? `${pagination.total} user${pagination.total !== 1 ? 's' : ''} across all organizations`
              : 'Loading…'}
          </p>
        </div>
      ...
      {!loading && users.length > 0 && <StatsStrip users={users} />}
                  ...
                  <tr key={u._id} className="table-row">
                    ...
                    <td className="table-td">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${u.isActive ? 'bg-primary-600' : 'bg-brand-300'}`}
                        >
