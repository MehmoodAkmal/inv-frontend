...
export default function CustomSelect({
  ...
  return (
    <div ref={rootRef} className={`relative inline-block align-top ${widthClassName} ${wrapperClassName}`}>
      <button
        id={controlId}
        type="button"
        disabled={disabled}
      ...
      >
        <span className={selected ? "truncate" : "truncate text-brand-400"}>{selected?.label ?? "Select an option"}</span>
        <svg className={`h-4 w-4 shrink-0 text-brand-500 transition-transform ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 9l6 6 6-6" />
        </svg>
      </button>
      {open && !disabled && (
        <div className="absolute z-50 mt-1 max-h-60 w-full min-w-max overflow-y-auto rounded-lg border border-brand-200 bg-white py-1 shadow-card-lg" role="listbox" aria-labelledby={controlId}>
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
            ...
            >
              ...
              {option.value === String(value) && <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
            </button>
          ))}
        </div>
      )}
